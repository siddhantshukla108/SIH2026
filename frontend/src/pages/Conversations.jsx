    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-slide-up w-full overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          <Link to="/dashboard" className="p-2 bg-[var(--color-shayak-beige)] hover:bg-[var(--color-shayak-rust-light)] rounded-xl text-[var(--color-shayak-sidebar)] transition-colors hidden sm:block">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="font-serif text-3xl font-bold text-[var(--color-shayak-sidebar)] tracking-tight">Recent Conversations</h1>
      ...
      <main className="w-full">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div className="relative w-full sm:w-auto">
            ...
            <input type="text" placeholder="Search conversations..." className="pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl outline-none focus:border-[var(--color-shayak-rust)] focus:ring-2 focus:ring-[var(--color-shayak-rust-light)] shadow-sm text-sm font-medium w-full sm:w-72 text-gray-700 transition-all" />
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm overflow-hidden border border-gray-100">
          {loading ? (
            <div className="p-12 flex flex-col items-center justify-center text-gray-400">
              <div className="w-8 h-8 border-4 border-gray-200 border-t-[var(--color-shayak-rust)] rounded-full animate-spin mb-4"></div>
              <p className="font-medium text-gray-500">Loading records...</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-[var(--color-shayak-bg)] border-b border-gray-100 text-gray-500 font-semibold uppercase tracking-wider text-[11px]">
                  <tr>
                  ...
                  {conversations.map((c) => (
                    <tr key={c._id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        <span className="capitalize font-semibold text-[var(--color-shayak-sidebar)] bg-[var(--color-shayak-beige)] px-3 py-1 rounded-lg text-xs">
                          {c.channel}
                        </span>
                      </td>
                      ...
                      <td className="py-4 px-6">
                        <div className="text-[var(--color-shayak-sidebar)] font-bold text-[14px]">
                      ...
                      <td className="py-4 px-6 text-right">
                        {c.status === 'completed' ? (
                          ...
                          <span className="inline-flex items-center px-3 py-1 rounded-xl text-xs font-bold bg-[var(--color-shayak-yellow)]/10 text-amber-700 border border-[var(--color-shayak-yellow)]/30">{c.state}</span>
                        )}
                        {c.isDemo && (
